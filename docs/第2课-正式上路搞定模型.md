...
- 第三个思路：我们可以借助Llama-index的openai-like包，来封装任何openai类似的大模型。不过这个包并没有实现嵌入模型接口，需要自己来实现。
- 第四个思路：我们可以在本地安装Ollama，在本地安装好模型，然后在Llama-index中使用Ollama的服务。



# 第一个思路，使用Llama-index为智谱构建的专门的包，直接安装最新版本即可。
这种方法对智谱来说最友好，但是对于国内的大模型，Llama-index只为智谱构建了专用包，其他厂家的大模型，我们就没办法这样使用了。
```bash
...
# 第三个思路：利用openai-like的包，来封装任何openai类似的大模型
这个思路的缺点很明显，只有对话模型，没有嵌入模型。需要手搓一个嵌入模型。
...
class ZhipuEmbeddings(BaseEmbedding):
    client: ZhipuAI = Field(default_factory=lambda: ZhipuAI(api_key=os.environ['ZHIPU_API_KEY'] ))
